 ...
 * George
 *
 * $Id: kwayrefine.c,v 1.4 2008/06/19 21:52:54 kddevin Exp $
 */
...
/* KDDKDD 2/7/2007 :  I think this assert should be   */
/* KDDKDD 2/7/2007 :  wspace->nlarge >= 2*xadj[nvtxs] */
...
/* KDDKDD  wspace->core (and, thus, wspace->degrees) is allocated with size
   KDDKDD  proportional to the number of local edges in the original graph.
   KDDKDD  If, as a result of the coarsening, the number of local edges
   KDDKDD  INCREASES, the assignments to edegrees below produce an 
   KDDKDD  array-bounds access error (ABW).  If the ABW is large enough,
   KDDKDD  other memory gets messed up and a segfault occurs when the 
   KDDKDD  memory is later freed.  
   KDDKDD  Purify detects the ABWs and later ABRs.  The code below prints
   KDDKDD  a message when an illegal access occurs.
   KDDKDD  Setting Zoltan parameter SCATTER_GRAPH to 3 is a potential 
   KDDKDD  workaround.
   KDDKDD  6/2008 for Arno Candel, SLAC.  See Zoltan-dev for more details.

if (edegrees+k > wspace->core+wspace->maxcore)
{int mype; 
MPI_Comm_rank(MPI_COMM_WORLD, &mype);
printf("%d KDDKDD ABW vtx=%d other=%d k=%d  wspace->degrees=%x:%x edegrees=%x edegrees+k=%x \n", mype, i, other, k, wspace->degrees, wspace->core+wspace->maxcore, edegrees, edegrees+k);
}
   KDDKDD */ 
