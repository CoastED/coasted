...
contains
  ...
  subroutine calculate_scalar_galerkin_projection(state, s_field)
    ...
    real (kind=8):: t0, t1, t2, run1, run2, speedup
    ...
    ! All elements will have same loc and quadrature on same mesh
    nloc = ele_loc(s_field, 1)
    ngi = ele_ngi(s_field, 1)
    ...
    select case(continuity(s_field))
      ...
      case(-1)
        ...
        if(nloc==4 .and. ngi ==11) then
            call gp_discontinuous_4_11
        end if
...
! =============== optimised stuff here ===================================

#define NLOC 4
#define NGI 11
#define gp_discontinuous_NLOC_NGI gp_discontinuous_4_11

#include "opt/Field_Copies_Opt.F90"
...
#define NLOC 3
#define NGI 2
#define gp_discontinuous_NLOC_NGI gp_discontinuous_3_2
