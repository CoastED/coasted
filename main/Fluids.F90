...
contains
  ...
  SUBROUTINE FLUIDS()
    ...
    ! Turbine extensions
    type(vector_field) :: coord, vel, velSrc
    type(scalar_field) :: coord_x, coord_y, coord_z
    type(scalar_field) :: vel_x, vel_y, vel_z
    type(scalar_field) :: velSrc_x, velSrc_y, velSrc_z
    type(scalar_field) :: bladeDistField
    ...
    ! Check and optionally add required default fields and options
   ...
   ! set the remaining timestepping options, needs to be before any diagnostics are calculated
    ...
    ! Set dump number. Set by checkpointing, or can be set manually.
    ! Defaults to 0.
    dump_no=0
...
!   end if
    ...
    ! Checkpoint at start
     if(do_checkpoint_simulation(dump_no)) &
        ...
        protect_simulation_name=.false., postfix="" )
    ! Dump at start
    if( &
    ...
    timestep_loop: do
       ...
       if( &
          ...
          ! Intermediate dumps
          ! removed '_checkpoint_' postfix from checkpointed simulation (Angus)
       ...
       ! Food for thought: stick LES calculations here? Calculating SGS
       ! viscosity each nonlinear iteration is wasteful.
       ...
       nonlinear_iteration_loop: do  ITS=1,nonlinear_iterations
          ...
          if (has_vector_field(state(1), "VelocitySource")) then
            ...
            print*, "**** its:", its
             ...
             coord_y = extract_scalar_field_from_vector_field(coord, 2)
             coord_z = extract_scalar_field_from_vector_field(coord, 3)

             ! If we have VelocityCG, then Velocity is probably discontinuous
             ! galerkin. Use VelocityCG instead.
             if (has_vector_field(state(1), "VelocityCG")) then
                 vel = extract_vector_field(state(1), "VelocityCG")
             else
                ! Otherwise just use Velocity is usual
                 vel = extract_vector_field(state(1), "Velocity")
             end if
             vel_x = extract_scalar_field_from_vector_field(vel, 1)
             vel_y = extract_scalar_field_from_vector_field(vel, 2)
             vel_z = extract_scalar_field_from_vector_field(vel, 3)

             velSrc = extract_vector_field(state(1), "VelocitySource")
             velSrc_x = extract_scalar_field_from_vector_field(velSrc, 1)
             velSrc_y = extract_scalar_field_from_vector_field(velSrc, 2)
             velSrc_z = extract_scalar_field_from_vector_field(velSrc, 3)



             call get_option("/material_phase[0]/equation_of_state/fluids/linear/reference_density", referenceDensity)
          ...
          if(nonlinear_iterations > 1) then
             ...
             if (have_option("/timestepping/nonlinear_iterations/&
                              &tolerance")) then
               ewrite(1, *) "Nonlinear iteration change = ", change
               ewrite(1, *) "Nonlinear iteration tolerance = ", nonlinear_iteration_tolerance
...
!       end if
       ...
       if(have_option("/mesh_adaptivity/hr_adaptivity")) then
          ...
          if(do_adapt_mesh(current_time, timestep)) then
             ...
             ! Set global parameters for mesh status.
             ! CoordinateMesh only, strictly speaking, but since VelocityMesh
             ! and PressureMesh are derived from this, also holds true for
             ! them.
             new_mesh_connectivity = .true.
             new_mesh_geometry = .true.
  ...
  subroutine update_state_post_adapt(state, metric_tensor, dt, sub_state, nonlinear_iterations, nonlinear_iterations_adapt)
   ...
   ! Ocean boundaries (needs to happen before move_mesh_free_surface().
   ! If the mesh is moved this will then be called again)
    if (has_scalar_field(state(1), "DistanceToTop")) then
      if (.not. have_option('/geometry/ocean_boundaries')) then
