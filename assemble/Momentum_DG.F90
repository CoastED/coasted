...
contains
    ...
    ! called below.
#include "Construct_Elements_DG1_TET_QUAD_3_CDG.F90"
#include "Construct_Elements_DG1_TET_QUAD_4_CDG.F90"
#include "Construct_Elements_DG1_TET_QUAD_3_BASSI.F90"
#include "Construct_Elements_DG1_TET_QUAD_4_BASSI.F90"
...
!                end if
                ...
                elseif(U%dim==3 .and. P%mesh%shape%degree==2 &
                    .and. have_viscosity .and. viscosity_scheme==BASSI_REBAY &
                    .and. ele_loc(U,1)==4 .and. ele_ngi(U,1)==11 ) then
                    print*, "Optimised DG assembly: Bassi-Rebay"
                    ...
                    colour_loop_bassi: do clr = 1, size(colours)
                        len = key_count(colours(clr))

                        !$OMP DO SCHEDULE(STATIC)
                        element_loop_bassi: do nnid = 1, len
                            ele = fetch(colours(clr), nnid)

                            call construct_momentum_elements_dg_DG1_TET_QUAD_4_BASSI( ele, big_m, rhs, &
                                & X, U, advecting_velocity, U_mesh, X_old, X_new, &
                                & u_shape, p_shape, q_shape, &
                                & Source, Buoyancy, hb_density, hb_pressure, gravity, Abs, Viscosity, &
                                & swe_bottom_drag, swe_u_nl, &
                        ...
                        end do element_loop_bassi
                    !      !$OMP END DO

                    end do colour_loop_bassi
                    !    print*, "after assembly loop"
...
subroutine construct_momentum_element_dg(ele, big_m, rhs, &
    ...
    real, dimension(U%dim) :: ele_centre, neigh_centre, &
    ...
    ! LES - sp911
    logical, intent(in) :: have_les, have_isotropic_les
    real, intent(in) :: smagorinsky_coefficient
    ...
    ! print*, "ngi:", ele_ngi(U,ele)
    ! print*, "ele_and_faces_loc:", ele_and_faces_loc(U,ele)
