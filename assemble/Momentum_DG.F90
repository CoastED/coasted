...
module momentum_DG
  ...
  use omp_lib
    ...
    !$OMP THREADPRIVATE(visc_grad_dot_u)
    ...
    !$OMP THREADPRIVATE(u_ele, p_ele, x_ele)
    ...
    !$OMP THREADPRIVATE(u_face_l, u_mesh_glno, u_face_glno_1, u_face_glno_2, Rho_face_glno_1, x_face_glno_1)
    !$OMP THREADPRIVATE(q_face_l)
    ...
    !$OMP THREADPRIVATE(face_dirichlet)
...
subroutine allocateDGAssemblyArrays()
    ...
    allocate( u_ele(opNloc) )
    allocate( p_ele(opNloc) )
    allocate( x_ele(opNloc) )
    ...
    allocate( u_face_l(opFloc) )
    allocate( u_mesh_glno(opFloc) )
    allocate( u_face_glno_1(opFloc) )
    allocate( u_face_glno_2(opFloc) )
    allocate( Rho_face_glno_1(opFloc) )
    allocate( x_face_glno_1(opFloc) )
    allocate( q_face_l(opFloc) )
...
subroutine deallocateDGAssemblyArrays()
    ...
    deallocate( u_face_l )
    deallocate( u_mesh_glno )
    deallocate( u_face_glno_1 )
    deallocate( u_face_glno_2 )
    deallocate( Rho_face_glno_1 )
    deallocate( x_face_glno_1 )
    deallocate( q_face_l )
