...
module sam_integration
  ...
  subroutine transfer_detectors(old_mesh, new_positions, node_ownership)
    ...
    ! This line causes a mysterious internal Intel compiler error - Angus
    ! nreceives = invert_comms_sizes(nsends, communicator = communicator)

    ! Replaced with this - which is all invert(comms_sizes() really does !
    call mpi_alltoall(nsends, 1, getpinteger(), nreceives, 1, getpinteger(), communicator, ierr)
