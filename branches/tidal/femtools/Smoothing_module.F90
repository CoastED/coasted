...
module smoothing_module
  ...
  public:: les_length_scale_tensor_bull, les_length_scales_squared_roman
...
contains
  ...
  ! Specialised length scale tensor for LES.
  ! From J. Bull (2012), Turbulence, Heat and Mass Transfer.

  subroutine les_length_scale_tensor_bull(positions, ele, lent)
    !! Computes a length scale tensor to be used in LES (units are in length^2)

    ! positions field must _only_ be linear; ie. the Coordinate mesh. This makes
    ! sense, since we are calculating this tensor based upon lengths.
    type(vector_field), intent(in) :: positions
    integer :: ele

    !! the resulting tensor (dim x dim )
    real, dimension(:, :) :: lent

    real, dimension(positions%dim, ele_loc(positions,ele)) :: X_val
    real, dimension(positions%dim, positions%dim) :: M, M_inv

    integer :: i, dim
    real :: dx(positions%dim), vol, twonorm, scaling
    ...
    do i=1, dim
       ...
       twonorm = twonorm + dx(i)*dx(i)
    end do

    ! As per Bull, scale down length to stop excessive turbulent diffusion,
    ! and match scalar LES case

    vol = element_volume(positions, ele)
    scaling = 1.0 ! (vol**0.6666666666) / twonorm

    M = outer_product(dx, dx)

    ! From alpha^2, where alpha = 2.0
    lent = 4.0 *scaling * M

  end subroutine les_length_scale_tensor_bull


  ! Horizontal and vertical lengthscales for anisotropic meshes,
  ! from Roman (2010). Only works for 3D linear tets.

  subroutine les_length_scales_squared_roman(positions, ele, horzSq, vertSq)
    type(vector_field), intent(in) :: positions
    integer :: ele

    real :: horzSq, vertSq
    real, dimension(ROMAN_MESH_DIM, ROMAN_NLOC) :: X_val
    real, dimension(ROMAN_MESH_DIM) :: dx
  ...
  end subroutine les_length_scales_squared_roman
