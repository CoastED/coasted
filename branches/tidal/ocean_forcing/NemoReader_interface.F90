...
module nemo_v2
  ...
  interface
     ...
     subroutine get_nemo_variables_c(time, X, Y, Z, DEPTH, Te, Sa, U, V, W, SSH, NNodes) bind(c)
       ...
       real(c_float), dimension(*), intent(in) :: X, Y, Z, DEPTH
       real(c_float), dimension(*), intent(out) :: Te, Sa, U, V, W, SSH
#endif
...
contains
  ...
  subroutine nemo_v2_GetScalars(longitude, latitude, p_depth, scalars)
    ...
    real (kind=8), dimension(size(scalars)) :: scalars_double
    integer :: i

    call nemo_v2_GetScalars_c(dble(longitude), dble(latitude), dble(p_depth), scalars_double)

    do i=1, size(scalars_double)
        scalars(i) = scalars_double(i)
    end do
#endif
...
#else
    real(real_4), intent(in) :: time
    real(kind=8) :: time_r8

    time_r8=time

    call nemo_v2_SetTimeSeconds_c(time_r8)
#endif
