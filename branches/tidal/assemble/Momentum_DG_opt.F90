...
contains
    ...
    subroutine construct_momentum_dg(u, p, rho, x, &
        ...
        ! LES - sp911
        logical :: have_les = .false., have_van_driest = .false., have_vel_cg=.false.
         ...
         &/compact_discontinuous_galerkin"                )) then
                ...
                if( have_les) then
                    if(partial_stress) then
                        ...
                        if (stat/=0) then
                            FLAbort("Can't do tensor DG LES without a DGLESTensorEddyViscosity field")
                        else
                        ...
                        if (stat/=0) then
                            ...
                            ewrite(1,*) "Found DistanceToWall field"
                        end if
    ...
    if (have_coriolis) then
       call set_coriolis_parameters
                ...
                print*, "**** DG - time spent in assemble:", assemble_dt
                print*, "**** DG - time since last call:", total_dt
                print*, "**** DG - % in assemble:", percent_dg
            ...
            subroutine les_tensor_viscosity_roman_elements(X, U, u_cg, p, uGrad, &
                ...
                real, dimension(MESH_DIM, MESH_DIM, NLOC) :: ug, viscEle
                type(element_type), pointer :: u_shape
                integer, dimension(:), pointer :: neigh
    
                ! Loop gubbins
                real, dimension(MESH_DIM, MESH_DIM, NLOC):: centralEleLesVisc, neighLesVisc, workLesVisc
                real, dimension(MESH_DIM, MESH_DIM) :: les_filter_width, lesViscNode, avLesVisc
                ...
                real, dimension(MESH_DIM, MESH_DIM, NLOC) :: tensor_visc
                real, dimension(MESH_DIM, NLOC) :: x_val
                ...
                element_loop: do ele = 1, nElements
                    ...
                    !          do i=1, MESH_DIM
                    !              do n=1, NLOC
                    !                  lesViscNode(i,MESH_DIM) = lesViscNode(i,MESH_DIM) + sgsVert(n)
                    !              end do
                    !          end do
                    ...
                    do i=1, MESH_DIM
                        do n=1, NLOC
                            workLesVisc(i,MESH_DIM,n) =  sgsVert(n)
                        end do
                    end do
            ...
            subroutine construct_momentum_element_dg(ele, big_m, rhs, &
                ...
                have_les, smagorinsky_coefficient, eddy_visc, tensor_eddy_visc, &
                ...
                ! Bilinear forms.
                real, dimension(NLOC, NLOC) :: &
                    Coriolis_mat, rho_mat, rho_move_mat, mass_mat
                real, dimension(NLOC, NLOC) :: &
                    inverse_mass_mat
                real, dimension(mesh_dim(U), NLOC, &
                    NLOC) :: ele2grad_mat
                real, dimension(NLOC, NLOC) :: &
                    Advection_mat
                real, dimension(NLOC, NLOC) :: &
                    Source_mat
                real, dimension(MESH_DIM, NLOC, NLOC) :: &
                    Abs_mat
                real, dimension(MESH_DIM, MESH_DIM, NLOC, NLOC) :: &
                    Abs_mat_sphere
                real, dimension(MESH_DIM, NLOC) :: &
                    Abs_lump
                real, dimension(MESH_DIM, MESH_DIM, NLOC) :: &
                    Abs_lump_sphere
                real, dimension(NLOC) :: &
                    source_lump
                real, dimension(NLOC, NLOC) :: Q_inv
                real, dimension(MESH_DIM, NLOC, ele_and_faces_loc(U,ele)) ::&
                    & Grad_u_mat_q, Div_u_mat_q
                real, dimension(MESH_DIM,MESH_DIM,ele_and_faces_loc(U,ele),ele_and_faces_loc(U,ele)) ::&
                    & Viscosity_mat
                real, dimension(MESH_DIM, MESH_DIM, &
                    & NLOC) :: Viscosity_ele
                real, dimension(MESH_DIM, NLOC) :: x_val, x_val_2
                real, dimension(MESH_DIM, NLOC) :: u_val

                 ! \Int_{ele} N_i kappa N_j dV, used for CDG fluxes
                real, dimension(MESH_DIM,MESH_DIM, NLOC,NLOC) :: kappa_mat
                ...
                real, dimension(MESH_DIM, ele_ngi(U_nl, ele)) :: u_nl_q
                ...
                real, dimension(MESH_DIM, ele_and_faces_loc(U,ele)) :: big_m_diag_addto,&
                    & rhs_addto
    
                real, dimension(MESH_DIM, MESH_DIM, ele_and_faces_loc(U,ele), ele_and_faces_loc(U,ele)) :: big_m_tensor_addto
                logical, dimension(MESH_DIM, MESH_DIM) :: diagonal_block_mask, off_diagonal_block_mask
                ! Addto matrices for when subcycling is performed
                real, dimension(MESH_DIM, MESH_DIM, ele_and_faces_loc(U,ele), &
                ...
                ! Absorption matrices
                real, dimension(MESH_DIM, ele_ngi(u, ele)) :: absorption_gi
                real, dimension(MESH_DIM, MESH_DIM, ele_ngi(u, ele)) :: tensor_absorption_gi

                ! Add vertical velocity relaxation to the absorption if present
                real, intent(in) :: vvr_sf
                real, dimension(MESH_DIM,MESH_DIM,ele_ngi(u,ele)) :: vvr_abs
                real, dimension(MESH_DIM,ele_ngi(u,ele)) :: vvr_abs_diag
                ...
                real, dimension(MESH_DIM,MESH_DIM,ele_ngi(u,ele)) :: ib_abs
                real, dimension(MESH_DIM,ele_ngi(u,ele)) :: ib_abs_diag
                real, dimension(ele_loc(u,ele),ele_ngi(u,ele),mesh_dim(u)) :: dt_rho
                real, dimension(MESH_DIM,ele_ngi(u,ele)) :: grav_at_quads
                real, dimension(MESH_DIM, ele_ngi(u,ele)) :: grad_rho
                ...
                real, dimension(MESH_DIM, ele_ngi(u, ele)) :: grad_nvfrac_gi

                ! element centre and neighbour centre
                ! for IP parameters

                real, dimension(MESH_DIM) :: ele_centre, neigh_centre, &
            ...
            contains
                ...
                subroutine les_tensor_viscosity(tensor_visc)
                    real, dimension(mesh_dim(u), mesh_dim(u), ele_loc(u,ele)), intent(inout) :: tensor_visc
                    real, dimension(mesh_dim(u), mesh_dim(u), ele_loc(u,ele)) :: les_visc
                    real, dimension(mesh_dim(u), mesh_dim(u), ele_ngi(u,ele)) :: les_filter_width
                    real, dimension(mesh_dim(u), mesh_dim(u), ele_loc(u,ele)) :: g_nl
                    real, dimension(mesh_dim(u), mesh_dim(u)) :: s
                    real, dimension(ele_loc(u,ele)) :: s_mod
                    real, dimension(ele_loc(u,ele)) :: y_wall, y_plus
                    real, dimension(ele_loc(u,ele), ele_loc(u,ele)) :: M_inv

                    real :: filter_scale(ele_loc(u,ele)), iso_visc

                    integer :: i, j, k, uloc


                    !      ewrite(1,*) "*** les_tensor_viscosity()"

                    uloc = ele_loc(u, ele)

                    ! print*, "tensor_visc dim:", size(tensor_visc(:,1,1)), ",",size(tensor_visc(1,:,1)), ",",size(tensor_visc(1,1,:))
                    ...
                    !      print*, "dim lst:", size(du_t,3),size(du_t,3),size(du_t,2)
                    ...
                    if (associated(distance_to_wall)) then
                        y_wall = ele_val(distance_to_wall, ele)
                        do i=1, uloc
                            iso_visc = norm2( tensor_visc(:,:,i) )
                            y_plus(i) = y_wall(i) * sqrt(norm2(g_nl(:,:,i)+transpose(g_nl(:,:,i))))/sqrt(iso_visc)
                            filter_scale(i) = (1 - exp(-1.0*y_plus(i)/25.0))
                        end do
                    ...
                    les_filter_width= length_scale_tensor(du_t, ele_shape(u, ele))
                    ...
                    !print*, "result:", 4*length_scale_tensor(du_t, ele_shape(u,ele))
                    !      end if

                    ! New tactic. Calculate average LES viscosity tensor across element.

                    les_visc = 0.0
                    do i=1, uloc
                        les_visc(:,:,i) = les_visc(:,:,i) + &
                            filter_scale(i) * les_filter_width(:,:,1)*(smagorinsky_coefficient**2) * s_mod(i)
                    end do

                    les_visc = les_visc / uloc

                    !      print*, "les_visc:", les_visc
                    !      print*, "s_mod:", s_mod
                    !      print*, "smagorinsky_coefficient:", smagorinsky_coefficient
                ...
                ! LES tensor viscosity a la Roman (2010), Int. J. of Heat and Fluid Flow
                subroutine les_tensor_viscosity_roman(tensor_visc)
                    real, dimension(MESH_DIM, MESH_DIM, NLOC), intent(inout) :: tensor_visc
                    real, dimension(MESH_DIM, MESH_DIM, NLOC) :: les_visc
                    real, dimension(MESH_DIM, MESH_DIM) :: les_filter_width
                    real, dimension(MESH_DIM, MESH_DIM, NLOC) :: g_nl
                    real :: magStrainHorz, magStrainVert, iso_visc
                    real :: sgsHorz, sgsVert
                    real, dimension(NLOC) :: y_wall, y_plus, filter_scale
                    real, dimension(NLOC, NLOC) :: M_inv

                    integer :: i, j, n
                    real :: dx(MESH_DIM)
                    real :: lenHorzSq, lenVertSq

                    X_val=ele_val(X, ele)

                    !      print*, "**** les_tensor_viscosity_roman():"
                    ...
                    if (associated(distance_to_wall)) then
                        y_wall = ele_val(distance_to_wall, ele)
                        do n=1, NLOC
                            iso_visc = norm2( tensor_visc(:,:,n) )
                            y_plus(n) = y_wall(n) * sqrt(norm2(g_nl(:,:,n)+transpose(g_nl(:,:,n))))/sqrt(iso_visc)
                            filter_scale(n) = (1 - exp(-1.0*y_plus(n)/25.0))
                        end do
                    ...
                    ! Calculate magnitude of horizontal and vertical strain (see Roman)

                    magStrainHorz = 0.0
                    magStrainVert = 0.0
                    do n=1, NLOC
                        !          print*,"n:", n
                        !          print*, g_nl(1,1,n), g_nl(2,2,n), g_nl(1,2,n), g_nl(1,3,n), g_nl(3,3,n), g_nl(3,2,n)
                        magStrainHorz = magStrainHorz + &
                            sqrt( 2*g_nl(1,1,n)**2 + 2*g_nl(2,2,n)**2 + 4*g_nl(1,2,n)**2)

                        magStrainVert = magStrainVert + &
                            sqrt( 4*g_nl(1,3,n)**2 + 2*g_nl(3,3,n)**2 + 4*g_nl(3,2,n)**2)
                    end do

                    magStrainHorz = magStrainHorz / NLOC
                    magStrainVert = magStrainVert / NLOC

                    sgsHorz = (smagorinsky_coefficient**2) * lenHorzSq * magStrainHorz
                    sgsVert = (smagorinsky_coefficient**2) * lenVertSq * magStrainVert

                    !      print *, "lenHorzSq, lenVertSq:", lenHorzSq, lenVertSq
                    !      print *, "magStrainHorz, magStrainVert:", magStrainHorz, magStrainVert

                    ! Average out LES viscosity over element
                    les_visc = 0.0
                    do i=1, MESH_DIM
                        do j=1, MESH_DIM-1
                            les_visc(i,j,:) = les_visc(i,j,:) + filter_scale(:) * sgsHorz
                        end do
                    end do

                    do i=1, MESH_DIM
                        les_visc(i,MESH_DIM,:) = les_visc(i,MESH_DIM,:) + filter_scale(:) * sgsVert
                    end do
                  ...
                  ! Now spread some of LES visc to neighbours (filter)
                !      do ni=1, size(neigh)
                !        n=neigh(ni)
                !        viscosity(n) = viscosity(e) + 0.5*les_visc
                !      end do

                end subroutine les_tensor_viscosity_roman
            ...
            subroutine construct_momentum_interface_dg(ele, face, face_2, ni, &
                ...
                real, dimension(2,face_loc(U,face),NLOC) ::&
                    & primal_fluxes_mat
                ...
                real, dimension(mesh_dim(U),NLOC,NLOC) :: &
                    & normal_mat
                ...
                real, dimension(MESH_DIM, face_ngi(U_nl, face)) :: normal, u_nl_q,&
                ...
                logical, dimension(MESH_DIM) :: dirichlet
                ...
                !    print*, "@@ face_loc(U,face):", face_loc(U,face)
                !    print*, "@@ face_ngi(U,face):", face_ngi(U,face)
                !    print*, "@@ face_ngi(U_nl,face):", face_ngi(U_nl,face)
                !    print*, "@@ face_loc(P,face):", face_loc(P,face)
                ...
                if(present(viscosity)) then
                    ...
                    !    print*, "@@ Viscosity%dim(2):", Viscosity%dim(2)
            ...
            contains
                ...
                subroutine interior_penalty
                    ...
                    ! FIXME: JRM HACK
                    !kappa_gi = kappa_gi + ele_val_at_quad(tensor_eddy_visc, ele)
                ...
                subroutine local_assembly_cdg_face
                    ...
                    integer, dimension(face_loc(U,face)) :: U_face_loc
                    real, dimension(mesh_dim(U),NLOC,face_loc(U,face)) :: R_mat
