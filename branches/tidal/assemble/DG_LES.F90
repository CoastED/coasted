...
!        ! (This will be checked for elsewhere)
    ...
    subroutine calc_dg_sgs_tensor_viscosity(state, x, u)
        ...
        real :: rate_of_strain(mesh_dim(u), mesh_dim(u))
        real :: mag_strain_horz, mag_strain_vert
        real :: sgs_horz, sgs_vert, sgs_offdiag
        real, dimension(mesh_dim(u), mesh_dim(u)) :: sgs_ele_av, visc_turb, u_grad_node
...
!        ! (This will be checked for elsewhere)
        ...
        if(new_mesh_connectivity) then
            ...
            allocate(node_sum(mesh_dim(u), mesh_dim(u), num_nodes))
            allocate(node_vol_weighted_sum(mesh_dim(u), mesh_dim(u), num_nodes))
            allocate(node_visits(num_nodes))
            allocate(node_neigh_total_vol(num_nodes))
        end if
        ...
        do e=1, num_elements
            ...
            do ln=1, NLOC
                ...
                sgs_offdiag = sqrt(sgs_vert*sgs_horz)

                visc_turb = 0.0
                visc_turb(1,1) = sgs_horz
                visc_turb(2,2) = sgs_horz
                visc_turb(3,1:2) = sgs_offdiag
                visc_turb(1:2,3) = sgs_offdiag
