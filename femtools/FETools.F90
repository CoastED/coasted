...
contains
  ...
  pure function shape_rhs(shape, detwei)
    !!<             /
  ...
  pure function shape_vector_rhs(shape,vector,detwei)
    !!<            /
  ...
  pure function shape_tensor_rhs(shape,tensor,detwei)
    !!<            /
  ...
  pure function shape_tensor_dot_vector_rhs(shape,tensor,vector,detwei)
    !!<            /
    !!<  Calculate | shape tensor detwei dV 
    !!<            /
    !!<
    ...
    real, dimension(:,:), intent(in) :: vector  !dim2 x ngi
  ...
  pure function dshape_dot_vector_rhs(dshape, vector, detwei)
    !!<             /
  ...
  pure function dshape_dot_tensor_rhs(dshape,tensor,detwei)
    !!<            /
  ...
  pure function shape_shape(shape1, shape2, detwei)
    ...
    do concurrent(iloc=1:shape1%loc,jloc=1:shape2%loc)
       ! Main mass matrix.
  ...
  ! JRM: Is this worth a try?
  subroutine shape_shape_fast(shape1, shape2, detwei, shape_shape)
    !!< For each node in each element shape1, shape2 calculate the
    !!< coefficient of the integral int(shape1shape2)dV.
    !!<
    !!< In effect, this calculates a mass matrix.
    ...
    real, dimension(:), intent(in) :: detwei
    ! JRM: Look, no malloc!
    real, dimension(:,:), intent(inout) :: shape_shape
  ...
  end subroutine shape_shape_fast

  pure function shape_shape_vector(shape1, shape2, detwei, vector)
    !!< For each node in each element shape1, shape2 calculate the
  ...
  pure function shape_shape_tensor(shape1, shape2, detwei, tensor)
    !!< For each node in each element shape1, shape2 calculate the
    ...
    do concurrent(iloc=1:shape1%loc,jloc=1:shape2%loc,i=1:size(tensor,1),j=1:size(tensor,2))
       ! Main mass matrix.
  ...
  pure function shape_shape_vector_outer_vector(shape1, shape2, detwei,vector1,vector2)
    !!< For each node in each element shape1, shape2 calculate the
    ...
    do concurrent(iloc=1:shape1%loc,jloc=1:shape2%loc,i=1:size(vector1,1),j=1:size(vector2,1))
       ! Main mass matrix.
  ...
  pure function dshape_rhs(dshape, detwei)
    !!<            /
    !!<  Calculate | dshape detwei dV 
    !!<            /
    !!<
...
#ifdef INLINE_MATMUL
    do concurrent(iloc=1:shape%loc,jloc=1:dshape_loc,idim=1:dim)
       shape_dshape(idim,iloc,jloc)= sum(detwei * dshape(jloc,:,idim) * shape%n(iloc,:))
    end do
#else
    do concurrent(iloc=1:shape%loc,jloc=1:dshape_loc)
       ! Main matrix.
  ...
  pure function dshape_shape(dshape, shape, detwei)
    !!< For each node in element shape and transformed gradient dshape, 
    ...
    do concurrent(iloc=1:dshape_loc,jloc=1:shape%loc)
       ! Main matrix.
  ...
  pure function dshape_dot_dshape(dshape1, dshape2, detwei) result (R)
    !!<            /          
    ...
    select case(dim)
      case(3)
        do gi=1,ngi
           do concurrent(iloc=1:loc1,jloc=1:loc2)
  ...
  pure function shape_vector_outer_dshape(&
    ...
    do concurrent(iloc=1:shape%loc,jloc=1:dshape_loc,i = 1:dim1,j = 1:dim2)
       ! Main matrix.
  ...
  pure function dshape_outer_vector_shape( &
    ...
    do concurrent(iloc=1:dshape_loc,jloc=1:shape%loc,i = 1:dim1,j = 1:dim2)
       ! Main matrix.
       tensor(i,j,iloc,jloc)= &
  ...
  pure function dshape_outer_dshape(dshape1, dshape2, detwei) result (R)
    ...
    do concurrent(iloc=1:loc1,jloc=1:loc2,i=1:dim1,j=1:dim2)
  ...
  pure function dshape_diagtensor_dshape(dshape1, tensor, dshape2, detwei) result (R)
    !!<
  ...
  pure function dshape_vector_dshape(dshape1, vector, dshape2, detwei) result (R)
    !!<
  ...
  pure function dshape_tensor_dshape(dshape1, tensor, dshape2, detwei) result (R)
    !!<
  ...
  function dshape_outer_tensor_dshape(dshape1, tensor, dshape2, detwei) result (R)
      !!<
      ...
      real, dimension(size(dshape1,1),size(dshape2,2)) :: temp
      ...
      do gi=1,ngi
          temp = matmul(dshape1(:,gi,:), tensor(:,:,gi)) * detwei(gi)
          forall(iloc=1:loc1,jloc=1:loc2,idim=1:dim)
              r(:,idim,iloc,jloc)=r(:,idim,iloc,jloc) &
                  +dshape2(jloc,gi,:)*temp(iloc,idim)
          end forall
      end do

  end function dshape_outer_tensor_dshape

function dshape_tensor_outer_dshape(dshape1, tensor, dshape2, detwei) result (R)
    !!<
    ...
    real, dimension(size(dshape1,3),size(dshape2,1)) :: temp
    ...
    do gi=1,ngi
       temp = matmul(tensor(:,:,gi),transpose(dshape2(:,gi,:)))*detwei(gi)
       forall(iloc=1:loc1,jloc=1:loc2,idim=1:dim)
          r(:,idim,iloc,jloc)=r(:,idim,iloc,jloc) &
               +temp(:,jloc)*dshape1(iloc,gi,idim)
       end forall
    end do

  end function dshape_tensor_outer_dshape


  pure function dshape_dot_vector_shape(dshape, vector, shape, detwei) result (R)
    !!< 
  ...
  pure function dshape_dot_tensor_shape(dshape, tensor, shape, detwei) result (R)
    !!<          /
    ...
    do concurrent(iloc=1:dshape_loc,jloc=1:shape%loc,idim2=1:dim2)
  ...
  pure function shape_vector_dot_dshape(shape, vector, dshape, detwei) result (R)
    !!< 
    !!< Evaluate (Grad N1 dot vector) (N2)
    !!< 
   type(element_type), intent(in) :: shape
  ...
  pure function shape_curl_shape_2d(shape, dshape, detwei) result (R)
    !!<            /          
...
!!$
!!$    do concurrent(i=1:size(mass,1))
!!$       mass(i,i)=sum(mass(i,:))
!!$    end do
!!$
!!$    do concurrent(i=1:size(mass,1),j=1:size(mass,1), i/=j)
!!$       mass(i,j)=0.0
!!$    end do
!!$    
!!$  end subroutine lump

  pure function lumped(mass)
    !!< lumped mass.
