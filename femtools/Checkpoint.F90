...
contains
  ...
  subroutine checkpoint_simulation(state, prefix, postfix, cp_no, protect_simulation_name, &
    ...
    ! Always set to disable dump at start, and disable checkpointing at start.
    ! You don't want to overwrite previous simulation dumps when you resume.
    if(.not. have_option("/io/disable_dump_at_start")) then
        ewrite(1,*) "Disabling dumps at start"
        call add_option("/io/disable_dump_at_start", stat)
    end if


    if(have_option("/io/checkpointing/checkpoint_at_start")) then
        ewrite(1,*) "Disabling checkpoints at start"
        call delete_option("/io/checkpointing/checkpoint_at_start")
    end if

    ! Set initial dump number. This will make sure that we don't overwrite
    ! previous simulation dumps, now that we don't do append
    ! '_checkpoint' to file names.
    ! Note the cp_no+1 -- that's because we want start at the next number

    if( present(cp_no)) then
        call add_option("/io/last_dump_number", stat)
        call set_option("/io/last_dump_number", cp_no, stat)
    end if
